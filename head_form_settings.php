...
// Fetch assigned year
$head_info = $conn->query("SELECT assigned_year FROM head WHERE id = $head_id");
$assigned_year = ($head_info && $head_info->num_rows > 0) ? $head_info->fetch_assoc()['assigned_year'] : 'SE';

// Determine default semester based on year
$default_sem = 3;
if ($assigned_year == 'TE') $default_sem = 5;
if ($assigned_year == 'BE') $default_sem = 7;
...
if (isset($_POST['save_form'])) {
    ...
    if (is_array($decoded)) {
        ...
        if (count($labels) !== count(array_unique($labels))) {
            $msg = "<div id='alertMsg' class='alert-error'><i class='fa-solid fa-triangle-exclamation'></i> <strong>Validation Error:</strong> Duplicate field labels detected. Each field must have a unique label.</div>";
            ...
            $check = $conn->query("SELECT id FROM form_settings WHERE academic_year='$assigned_year' AND semester=$sem");
            if ($check->num_rows > 0) {
                $sql = "UPDATE form_settings SET is_form_open=$is_open, min_team_size=$min_size, max_team_size=$max_size, form_schema='$schema_escaped' WHERE academic_year='$assigned_year' AND semester=$sem";
            } else {
                $sql = "INSERT INTO form_settings (academic_year, semester, is_form_open, min_team_size, max_team_size, form_schema) VALUES ('$assigned_year', $sem, $is_open, $min_size, $max_size, '$schema_escaped')";
            }

            if ($conn->query($sql) === TRUE) {
                $msg = "<div id='alertMsg' class='alert-success'><i class='fa-solid fa-paper-plane'></i> Form configuration for $assigned_year (Sem $sem) published successfully!</div>";
                if (isset($_POST['is_ajax'])) send_ajax_response('success', "Form configuration for $assigned_year (Sem $sem) published!");
            } else {
...
$current_settings = $conn->query("SELECT * FROM form_settings WHERE academic_year='$assigned_year' AND semester=$selected_sem")->fetch_assoc();
...
<body>
    ...
    <div class="main-content">
            ...
            <div class="top-navbar-inner">
                <div class="top-navbar-left">
                    ...
                    <div>
                        ...
                        <p style="font-size: 13px; color: var(--text-light); margin:0;">Configure registration form for <?php echo htmlspecialchars($assigned_year); ?> Sem <?php echo $selected_sem; ?></p>
                    </div>
                </div>
        ...
        <div id="canvas-area">
            <form method="GET" style="margin-bottom: 20px; max-width:800px; margin: 0 auto 20px auto; text-align:center;">
                <label style="font-size: 14px; font-weight: 600; color: var(--text-dark); margin-right: 10px;">Select Semester:</label>
            ...
            <form method="POST" id="masterForm" class="ajax-form">
                ...
                <div class="builder-container">
                    ...
                    <div class="form-header-card">
                        <input type="text" class="form-title" value="Project Registration Form (<?php echo htmlspecialchars($assigned_year); ?> - Sem <?php echo $selected_sem; ?>)" readonly>
                        <p style="color:var(--text-light); font-size:14px; margin-top:5px;">This form is generated dynamically for <?php echo htmlspecialchars($assigned_year); ?> Semester <?php echo $selected_sem; ?> students.</p>
                        ...
                        <div style="margin-top: 30px; display:flex; align-items:center; justify-content:space-between; background:var(--input-bg); padding:15px 20px; border-radius:12px; border:1px solid var(--border-color);">
                            <div>
                                ...
                                <p style="font-size:12px; color:var(--text-light); margin-top:2px;">Turn off to lock the form for all <?php echo htmlspecialchars($assigned_year); ?> Sem <?php echo $selected_sem; ?> students.</p>
                            </div>
                            <label class="switch">
            ...
            document.querySelectorAll('.options-sortable-container').forEach(container => {
                new Sortable(container, {
                    ...
                    onEnd: function(evt) {
                        ...
                        // DOM is already updated by Sortable, data model is updated.
                    }
                });
            });
        }
        ...
        function openPreviewModal() {
            ...
            let html = `
                <div style="border-top: 10px solid var(--primary-green); background:var(--card-bg); border-radius:16px; padding:30px; margin-bottom:20px; box-shadow:var(--shadow); border-left:1px solid var(--border-color); border-right:1px solid var(--border-color); border-bottom:1px solid var(--border-color);">
                    <h1 style="font-size:26px; color:var(--text-dark); margin-bottom:5px;">Project Registration Form (${'<?php echo htmlspecialchars($assigned_year); ?>'} - Sem ${'<?php echo $selected_sem; ?>'})</h1>
