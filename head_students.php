...
if (isset($_POST['add_student'])) {
    ...
    if (moodle_id_in_use($conn, $m_id)) {
        ...
        $stmt = $conn->prepare("INSERT INTO student (moodle_id, password, full_name, academic_year, current_semester, division, phone_number, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'Active')");
        $stmt->bind_param("ssssiss", $m_id, $hashed_pass, $name, $assigned_year, $sem, $div, $phone);
        if ($stmt->execute()) {
...
if (isset($_POST['edit_student'])) {
    ...
    $m_id = $conn->real_escape_string($_POST['moodle_id']);
    $name = $conn->real_escape_string($_POST['full_name']);
    $year = isset($_POST['academic_year']) ? $conn->real_escape_string($_POST['academic_year']) : $assigned_year;
    $div = $conn->real_escape_string($_POST['division']);
    $phone = $conn->real_escape_string($_POST['phone_number']);
    $sem = (int)$_POST['current_semester'];
    ...
    $id_int = (int)$id;
    $stmt_c = $conn->prepare("SELECT moodle_id, academic_year, status FROM student WHERE id = ?");
    ...
    if ($current_mid !== null && $m_id !== $current_mid) {
        ...
        if (($current_year !== null && $current_year !== $year) || ($current_status !== 'Disabled' && $status === 'Disabled')) {
            $stmt_l = $conn->prepare("SELECT id, group_name FROM projects WHERE leader_id = ? AND is_archived = 0");
            $stmt_l->bind_param("i", $id_int);
            $stmt_l->execute();
            $check_leader = $stmt_l->get_result();
            $stmt_l->close();
            ...
            if ($check_leader && $check_leader->num_rows > 0) {
                ...
                if (isset($_POST['new_leader_id']) && !empty($_POST['new_leader_id'])) {
                    ...
                    $stmt_u1 = $conn->prepare("UPDATE projects SET leader_id = ? WHERE id = ?");
                    $stmt_u1->bind_param("ii", $new_leader_id, $p_id);
                    $stmt_u1->execute();
                    $stmt_u1->close();

                    $stmt_u2 = $conn->prepare("UPDATE project_members SET is_leader = 0 WHERE project_id = ?");
                    $stmt_u2->bind_param("i", $p_id);
                    $stmt_u2->execute();
                    $stmt_u2->close();

                    $stmt_u3 = $conn->prepare("UPDATE project_members SET is_leader = 1 WHERE project_id = ? AND student_id = ?");
                    $stmt_u3->bind_param("ii", $p_id, $new_leader_id);
                    $stmt_u3->execute();
                    $stmt_u3->close();

                    $stmt_del = $conn->prepare("DELETE FROM project_members WHERE student_id = ? AND project_id = ?");
                    $stmt_del->bind_param("ii", $id_int, $p_id);
                    $stmt_del->execute();
                    $stmt_del->close();
                } else {
                    $stmt_m = $conn->prepare("SELECT s.id, s.full_name, s.moodle_id FROM project_members pm JOIN student s ON pm.student_id = s.id WHERE pm.project_id = ? AND s.id != ?");
                    $stmt_m->bind_param("ii", $p_id, $id_int);
                    $stmt_m->execute();
                    $members_q = $stmt_m->get_result();
                    $stmt_m->close();
        ...
        if ($proceed_with_update) {
            ...
            if (!empty($new_pass)) {
                ...
                $stmt = $conn->prepare("UPDATE student SET full_name=?, academic_year=?, current_semester=?, division=?, phone_number=?, status=?, password=? WHERE id=?");
                $stmt->bind_param("ssissssi", $name, $year, $sem, $div, $phone, $status, $h, $id_int);
            } else {
                $stmt = $conn->prepare("UPDATE student SET full_name=?, academic_year=?, current_semester=?, division=?, phone_number=?, status=? WHERE id=?");
                $stmt->bind_param("ssisssi", $name, $year, $sem, $div, $phone, $status, $id_int);
            }
...
$sql = "SELECT s.*, p.id as project_id, p.group_name, p.is_locked, p.final_topic
        ...
        WHERE s.deleted_at IS NULL AND s.academic_year = '$assigned_year' AND (s.status = 'Active' OR s.status IS NULL)
                ...
                <div class="card" style="padding:0; overflow:hidden;">
                    ...
                    <tbody style="padding:15px;">
                        ...
                        <tr class="student-row">
                            ...
                            <td style="padding:15px;">
                                <span style="font-size:14px; font-weight: 600; color: var(--text-dark);">Sem <?php echo $row['current_semester']; ?> (Div <?php echo htmlspecialchars($row['division']); ?>)</span><br>
            ...
            <div id="section-bulk" class="mgmt-section">
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap:20px;">
                    ...
                    <div class="card" style="text-align:left; margin:0;">
                        <h3 style="font-size:18px; margin-bottom:15px; color:var(--text-dark); display:flex; align-items:center; gap:10px;"><i class="fa-solid fa-file-import" style="color:var(--primary-green);"></i> Bulk CSV Import (<?php echo $assigned_year; ?>)</h3>
                        <p style="font-size:13px; color:var(--text-light); margin-bottom:20px;">Upload a CSV file for <strong><?php echo $assigned_year; ?></strong> students. Headers: <code>moodle_id</code>, <code>full_name</code>, <code>division</code>, <code>phone_number</code>.</p>
                        
                        <form id="importCsvForm" onsubmit="handleBulkAction(event, 'import_csv')">
                            <input type="hidden" id="import_year" value="<?php echo $assigned_year; ?>">
                            <div style="margin-bottom:15px;">
                                ...
                                <select id="import_sem" class="filter-select" style="width:100%;">
                                    <?php 
                                        $sem_opts = ($assigned_year == 'SE') ? [3,4] : (($assigned_year == 'TE') ? [5,6] : [7,8]);
                                        foreach($sem_opts as $o) echo "<option value='$o'>Sem $o</option>";
                                    ?>
                                </select>
                            </div>
                    ...
                    <div class="card" style="text-align:left; border-top: 5px solid #EF4444; margin:0;">
                        <h3 style="font-size:18px; margin-bottom:15px; color:#EF4444; display:flex; align-items:center; gap:10px;"><i class="fa-solid fa-user-minus"></i> Bulk Removal (<?php echo $assigned_year; ?>)</h3>
                        <p style="font-size:13px; color:var(--text-light); margin-bottom:20px;">Disable all <strong><?php echo $assigned_year; ?></strong> students for a specific semester.</p>
                        
                        <form id="bulkDeleteForm" onsubmit="handleBulkAction(event, 'bulk_delete')">
                            <input type="hidden" id="delete_year" value="<?php echo $assigned_year; ?>">
                            <div style="margin-bottom:15px;">
                                <label style="font-size:12px; font-weight:600; color:var(--text-light); display:block; margin-bottom:5px;">Target Semester</label>
                                <select id="delete_sem" class="filter-select" style="width:100%;">
                                    <?php foreach($sem_opts as $o) echo "<option value='$o'>Sem $o</option>"; ?>
                                </select>
                            </div>
                            <div style="background:rgba(239,68,68,0.05); padding:15px; border-radius:12px; border:1px solid rgba(239,68,68,0.2); margin-bottom:20px;">
                                <p style="font-size:12px; color:#991B1B;"><i class="fa-solid fa-triangle-exclamation"></i> This action will move students to the 'Removed' list.</p>
                            </div>
            ...
            <div id="section-disabled" class="mgmt-section">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; flex-wrap:wrap; gap:10px;">
                    <div style="display:flex; gap:10px;">
                        <input type="hidden" id="inactive_year" value="<?php echo $assigned_year; ?>">
                        <select id="inactive_sem" class="filter-select" onchange="fetchInactiveStudents()">
                            <option value="0">All Semesters (<?php echo $assigned_year; ?>)</option>
                            <?php foreach($sem_opts as $o) echo "<option value='$o'>Sem $o</option>"; ?>
                        </select>
                    </div>
                ...
                <div class="card" style="padding:0; overflow:hidden; margin:0;">
                    <table>
                        ...
                        <tbody id="inactiveStudentsList">
                            <tr><td colspan="3" style="text-align: center; color: var(--text-light); padding: 40px;"><i class="fa-solid fa-spinner fa-spin" style="font-size:30px; margin-bottom:15px;"></i><br>Loading records...</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    <script>
        ...
        async function handleBulkAction(event, action) {
            ...
            if(action === 'bulk_delete') {
                if(!confirm('CRITICAL WARNING: This will DISABLE and REMOVE all students for the selected Semester. They will lose access immediately. Are you sure?')) return;
            }
            ...
            try {
                ...
                if(result.status === 'success') {
                    if(result.students.length === 0) {
                        list.innerHTML = '<tr><td colspan="3" style="text-align:center; padding:40px; color:var(--text-light);"><i class="fa-solid fa-user-check" style="font-size:30px; margin-bottom:10px; opacity:0.3;"></i><br>No inactive students found.</td></tr>';
                        return;
                    }
                        ...
                        html += `
                            <tr>
                                ...
                                <td style="padding:15px;">
                                    ...
                                    <span style="font-size:11px; color:var(--text-light);">Sem ${s.current_semester}</span>
                                </td>
