...
if (isset($_POST['add_guide'])) {
   $m_id = $conn->real_escape_string($_POST['moodle_id']); 
    ...
    if (moodle_id_in_use($conn, $m_id)) {
        ...
        if (isset($_POST['is_ajax'])) send_ajax_response('success', 'Mentor added successfully!', ['callback' => 'refreshDashboard', 'closeModal' => 'addGuideModal']);
    }
}
...
if (isset($_POST['edit_guide'])) {
    ...
    $current = $conn->query("SELECT moodle_id FROM guide WHERE id = $id")->fetch_assoc();
    ...
    if ($current_mid !== null && $m_id !== $current_mid) {
        ...
        $conn->query("UPDATE guide SET full_name='$new_name', contact_number='$contact' WHERE id=$id");

        if ($current_mid !== null) {
            $mid_for_sync = $conn->real_escape_string($current_mid);
            $conn->query("UPDATE head SET full_name='$new_name' WHERE moodle_id='$mid_for_sync'");
            $conn->query("UPDATE admin SET full_name='$new_name' WHERE moodle_id='$mid_for_sync'");
        }
        $msg = "<div class='alert-success'><i class='fa-solid fa-check'></i> Profile updated everywhere dynamically!</div>";
        if (isset($_POST['is_ajax'])) send_ajax_response('success', 'Profile updated successfully!', ['callback' => 'refreshDashboard', 'closeModal' => 'editGuideModal']);
    }
}
...
if (isset($_POST['make_head'])) {
    ...
    $check_year = $conn->query("SELECT id FROM head WHERE assigned_year = '$assigned_year' AND deleted_at IS NULL");
    if ($check_year->num_rows > 0) {
        ...
        if (isset($_POST['is_ajax'])) send_ajax_response('error', "$assigned_year already has a Year Head.");
    } else {
        $check_moodle = $conn->query("SELECT id, deleted_at FROM head WHERE moodle_id = '$m_id'");
        if ($check_moodle->num_rows > 0) {
            $h_row = $check_moodle->fetch_assoc();
            if ($h_row['deleted_at'] === null) {
                $msg = "<div class='alert-error'><i class='fa-solid fa-circle-exclamation'></i> Error: This person is already an active Head.</div>";
            } else {
                // Reactivate and update
                $conn->query("UPDATE head SET password='$pass', full_name='$name', assigned_year='$assigned_year', deleted_at=NULL, status='Active' WHERE moodle_id='$m_id'");
                $msg = "<div class='alert-success'><i class='fa-solid fa-crown'></i> $name was successfully re-promoted to $assigned_year Year Head!</div>";
            }
        } else {
            $conn->query("INSERT INTO head (moodle_id, password, full_name, assigned_year, status) VALUES ('$m_id', '$pass', '$name', '$assigned_year', 'Active')");
            $msg = "<div class='alert-success'><i class='fa-solid fa-crown'></i> $name was successfully promoted to $assigned_year Year Head!</div>";
        }
        if (isset($_POST['is_ajax'])) send_ajax_response('success', 'Promotion successful!', ['callback' => 'refreshDashboard', 'closeModal' => 'makeHeadModal']);
    }
}
...
if (isset($_POST['delete_guide'])) {
    ...
    $conn->query("UPDATE upload_requests SET guide_id = NULL WHERE guide_id = $id");
    $conn->query("UPDATE guide SET deleted_at = NOW() WHERE id = $id");
    $msg = "<div class='alert-success'><i class='fa-solid fa-trash-can'></i> Mentor removed successfully!</div>";
    if (isset($_POST['is_ajax'])) send_ajax_response('success', 'Mentor removed successfully!', ['callback' => 'refreshDashboard']);
}
...
if (isset($_POST['add_head'])) {
    ...
    $check = $conn->query("SELECT id FROM head WHERE assigned_year = '$year' AND deleted_at IS NULL");
    if ($check->num_rows > 0) {
        ...
        if (isset($_POST['is_ajax'])) send_ajax_response('error', "A Head is already assigned to $year.");
    } else {
        $check_moodle = $conn->query("SELECT id, deleted_at FROM head WHERE moodle_id = '$m_id'");
        if ($check_moodle->num_rows > 0) {
            $h_row = $check_moodle->fetch_assoc();
            if ($h_row['deleted_at'] === null) {
                $msg = "<div class='alert-error'><i class='fa-solid fa-circle-exclamation'></i> Moodle ID '$m_id' is already an active Head!</div>";
            } else {
                // Reactivate
                $conn->query("UPDATE head SET password='$pass', full_name='$name', assigned_year='$year', deleted_at=NULL, status='Active' WHERE moodle_id='$m_id'");
                $msg = "<div class='alert-success'><i class='fa-solid fa-check-circle'></i> Year Head account reactivated and assigned successfully!</div>";
            }
        } else {
            if (moodle_id_in_use($conn, $m_id)) {
                $msg = "<div class='alert-error'><i class='fa-solid fa-circle-exclamation'></i> Moodle ID '$m_id' is already in use by another role!</div>";
                if (isset($_POST['is_ajax'])) send_ajax_response('error', "Moodle ID '$m_id' is already in use!");
            } else {
                $conn->query("INSERT INTO head (moodle_id, password, full_name, assigned_year, status) VALUES ('$m_id', '$pass', '$name', '$year', 'Active')");
                $msg = "<div class='alert-success'><i class='fa-solid fa-check-circle'></i> Year Head added successfully!</div>";
            }
        }
        if (isset($_POST['is_ajax'])) send_ajax_response('success', 'Year Head added successfully!', ['callback' => 'refreshDashboard', 'closeModal' => 'addHeadModal']);
    }
}
...
if (isset($_POST['edit_head'])) {
    ...
    $name = $conn->real_escape_string($_POST['full_name']);
    $year = $conn->real_escape_string($_POST['assigned_year']);
    $check = $conn->query("SELECT id FROM head WHERE assigned_year = '$year' AND id != $id AND deleted_at IS NULL");
    if ($check->num_rows > 0) {
        $msg = "<div class='alert-error'><i class='fa-solid fa-circle-exclamation'></i> Error: Another Head is already assigned to $year.</div>";
        if (isset($_POST['is_ajax'])) send_ajax_response('error', "Another Head is already assigned to $year.");
    } else {
        ...
        if ($current_mid !== null && $m_id !== $current_mid) {
            ...
            if ($current_mid !== null) {
                ...
                $conn->query("UPDATE admin SET full_name='$name' WHERE moodle_id='$mid_for_sync'");
            }
            $msg = "<div class='alert-success'><i class='fa-solid fa-pen'></i> Year Head info updated!</div>";
        }
        if (isset($_POST['is_ajax'])) send_ajax_response('success', 'Year Head info updated!', ['callback' => 'refreshDashboard', 'closeModal' => 'editHeadModal']);
    }
}
...
if (isset($_POST['delete_head'])) {
    $id = (int)$_POST['head_id'];
    $conn->query("UPDATE head SET deleted_at = NOW() WHERE id = $id");
    $msg = "<div class='alert-success'><i class='fa-solid fa-trash-can'></i> Year Head removed successfully!</div>";
    if (isset($_POST['is_ajax'])) send_ajax_response('success', 'Year Head removed successfully!', ['callback' => 'refreshDashboard']);
}

// ==============================================
//               FETCH DATA
// ==============================================
$guides = $conn->query("SELECT * FROM guide WHERE deleted_at IS NULL ORDER BY full_name");
$heads = $conn->query("SELECT * FROM head WHERE deleted_at IS NULL ORDER BY assigned_year DESC");
                            ...
                            <tr class="searchable-row">
                                ...
                                <td style="text-align:center; padding:15px; vertical-align:middle;">
                                    <div class="action-flex">
                                        ...
                                        <form method="POST" class="ajax-form" style="margin:0;" onsubmit="return confirm('Remove this guide? This will unassign them from ALL their current groups.');">
                            ...
                            <tr class="searchable-row">
                                ...
                                <td style="text-align:center; padding:15px; vertical-align:middle;">
                                    <div class="action-flex">
                                        ...
                                        <form method="POST" class="ajax-form" style="margin:0;" onsubmit="return confirm('Remove this Year Head permanently?');">
                                            <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
