...
if (isset($_POST['add_guide'])) {
    ...
    if (moodle_id_in_use($conn, $m_id)) {
        ...
        $sql = "INSERT INTO guide (moodle_id, password, full_name, contact_number, status) VALUES ('$m_id', '$pass', '$name', '$contact', 'Active')";
        if ($conn->query($sql) === TRUE) $msg = "<div class='alert-success'><i class='fa-solid fa-check-circle'></i> Guide added successfully!</div>";
    }
}
...
if (isset($_POST['edit_guide'])) {
    ...
    $id_int = (int)$id;
    $current = $conn->query("SELECT moodle_id FROM guide WHERE id = $id_int")->fetch_assoc();
    ...
    if ($current_mid !== null && $m_id !== $current_mid) {
        ...
        $sql = "UPDATE guide SET full_name='$name', contact_number='$contact' WHERE id=$id_int";
        if ($conn->query($sql) === TRUE) {
            // If this person is also a Head (same moodle_id), keep profile in sync.
            $mid_for_sync = $conn->real_escape_string($current_mid ?? $m_id);
            $conn->query("UPDATE head SET full_name='$name', contact_number='$contact' WHERE moodle_id='$mid_for_sync'");
...
// FETCH GUIDES
$sql = "SELECT * FROM guide ORDER BY full_name";
