...
$assigned_year = $head_info['assigned_year'];
...
if (isset($_POST['save_form'])) {
    ...
    $schema_json = $conn->real_escape_string($_POST['schema_json']);

    $check = $conn->query("SELECT id FROM form_settings WHERE academic_year='$assigned_year'");
    if ($check->num_rows > 0) {
        $sql = "UPDATE form_settings SET is_form_open=$is_open, min_team_size=$min_size, max_team_size=$max_size, form_schema='$schema_json' WHERE academic_year='$assigned_year'";
    } else {
        $sql = "INSERT INTO form_settings (academic_year, is_form_open, min_team_size, max_team_size, form_schema) VALUES ('$assigned_year', $is_open, $min_size, $max_size, '$schema_json')";
    }

    if ($conn->query($sql) === TRUE) {
        $msg = "<div id='alertMsg' class='alert-success'><i class='fa-solid fa-paper-plane'></i> Form configuration for $assigned_year published successfully!</div>";
    } else {
...
$current_settings = $conn->query("SELECT * FROM form_settings WHERE academic_year='$assigned_year'")->fetch_assoc();
...
<head>
    ...
    <title>Form Builder - Head Dashboard</title>
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <a href="head_form_settings.php" class="nav-link active"><i class="fa-brands fa-google"></i> Form Builder</a>
            ...
            <div class="top-navbar-inner">
                <div class="top-navbar-left">
                    ...
                    <div>
                        ...
                        <p style="font-size: 13px; color: var(--text-light); margin:0;">Configure registration form for <?php echo htmlspecialchars($assigned_year); ?></p>
                    </div>
                </div>
            ...
            <form method="POST" id="masterForm">
                ...
                <div class="builder-container">
                    ...
                    <div class="form-header-card">
                        <input type="text" class="form-title" value="Project Registration Form (<?php echo htmlspecialchars($assigned_year); ?>)" readonly>
                        <p style="color:var(--text-light); font-size:14px; margin-top:5px;">This form is generated dynamically for <?php echo htmlspecialchars($assigned_year); ?> students.</p>
                        ...
                        <div style="margin-top: 30px; display:flex; align-items:center; justify-content:space-between; background:var(--input-bg); padding:15px 20px; border-radius:12px; border:1px solid var(--border-color);">
                            <div>
                                ...
                                <p style="font-size:12px; color:var(--text-light); margin-top:2px;">Turn off to lock the form for all <?php echo htmlspecialchars($assigned_year); ?> students.</p>
                            </div>
                            <label class="switch">
    ...
    <script>
        ...
        function renderCanvas() {
            ...
            formFields.forEach((field, index) => {
                ...
                card.className = `q-card ${index === activeIndex ? 'active' : ''}`;
                ...
                if(field.type === 'team-members') {
                    card.innerHTML = `
                        ...
                        <div style="display:flex; gap:20px; background:var(--input-bg); padding:20px; border-radius:12px; border:1px solid var(--border-color);">
                            <div style="flex:1;">
                                ...
                                <input type="number" name="min_team_size" value="<?php echo $min; ?>" min="1" style="width:100%; padding:12px; border-radius:8px; border:1px solid var(--border-color); outline:none; font-family:inherit; background:var(--card-bg); color:var(--text-dark);">
                            </div>
                            <div style="flex:1;">
                                <label style="font-size:12px; font-weight:600; color:var(--text-light); margin-bottom:8px; display:block;">Maximum Size</label>
                                <input type="number" name="max_team_size" value="<?php echo $max; ?>" min="1" style="width:100%; padding:12px; border-radius:8px; border:1px solid var(--border-color); outline:none; font-family:inherit; background:var(--card-bg); color:var(--text-dark);">
                            </div>
                        </div>
                    `;
                } else {
                    ...
                    if (['select', 'radio', 'checkbox'].includes(field.type)) {
                        ...
                        optionsHtml += `
                            <div class="option-row" style="margin-top:10px;">
                                <i class="option-icon ${optIcon}" style="opacity:0.5;"></i>
                                <button type="button" class="add-option-btn" onclick="addOption(${index})"><i class="fa-solid fa-plus"></i> Add option</button>
                            </div>
                        `;
                    } else if (field.type === 'textarea') {
                        optionsHtml = `<div style="color:var(--text-light); font-size:14px; border-bottom:1px dashed var(--border-color); padding:10px 0; width:80%;">Long answer text</div>`;
                    ...
                    card.innerHTML = `
                        ...
                        <div class="q-footer">
                            <button type="button" class="q-action-icon" title="Move Up" onclick="moveField(${index}, -1)"><i class="fa-solid fa-arrow-up"></i></button>
                            <button type="button" class="q-action-icon" title="Move Down" onclick="moveField(${index}, 1)"><i class="fa-solid fa-arrow-down"></i></button>
        ...
        function moveField(idx, dir) {
            if (idx === 0) return; // Can't move team block
            let newIdx = idx + dir;
            if (newIdx <= 0 || newIdx >= formFields.length) return; 
            
            let temp = formFields[idx];
        ...
        function openPreviewModal() {
            ...
            let html = `
                <div style="border-top: 10px solid var(--primary-green); background:var(--card-bg); border-radius:16px; padding:30px; margin-bottom:20px; box-shadow:var(--shadow); border-left:1px solid var(--border-color); border-right:1px solid var(--border-color); border-bottom:1px solid var(--border-color);">
                    <h1 style="font-size:26px; color:var(--text-dark); margin-bottom:5px;">Project Registration Form (${'<?php echo htmlspecialchars($assigned_year); ?>'})</h1>
