...
if (isset($_POST['save_form'])) {
    ...
    $check = $conn->query("SELECT id FROM form_settings WHERE academic_year='$year'");
    if ($check->num_rows > 0) {
        $sql = "UPDATE form_settings SET is_form_open=$is_open, min_team_size=$min_size, max_team_size=$max_size, form_schema='$schema_json' WHERE academic_year='$year'";
    } else {
        $sql = "INSERT INTO form_settings (academic_year, is_form_open, min_team_size, max_team_size, form_schema) VALUES ('$year', $is_open, $min_size, $max_size, '$schema_json')";
    }

    if ($conn->query($sql) === TRUE) {
        $msg = "<div id='alertMsg' class='alert-success'><i class='fa-solid fa-paper-plane'></i> Form configuration for $year published successfully!</div>";
    } else {
...
$current_settings = $conn->query("SELECT * FROM form_settings WHERE academic_year='$selected_year'")->fetch_assoc();
    ...
    <div class="main-content">
        ...
        <div id="canvas-area">
            <form method="GET" style="margin-bottom: 20px; max-width:800px; margin: 0 auto 20px auto; text-align:center;">
                <label style="font-size: 14px; font-weight: 600; color: var(--text-dark); margin-right: 10px;">Editing Form For:</label>
            ...
            <form method="POST" id="masterForm">
                ...
                <div class="builder-container">
                    ...
                    <div class="form-header-card">
                        <input type="text" class="form-title" value="Project Registration Form (<?php echo htmlspecialchars($selected_year); ?>)" readonly>
                        <p style="color:var(--text-light); font-size:14px; margin-top:5px;">This form is generated dynamically for <?php echo htmlspecialchars($selected_year); ?> students.</p>
                        ...
                        <div style="margin-top: 30px; display:flex; align-items:center; justify-content:space-between; background:var(--input-bg); padding:15px 20px; border-radius:12px; border:1px solid var(--border-color);">
                            <div>
                                ...
                                <p style="font-size:12px; color:var(--text-light); margin-top:2px;">Turn off to lock the form for all <?php echo htmlspecialchars($selected_year); ?> students.</p>
                            </div>
                            <label class="switch">
        ...
        function openPreviewModal() {
            ...
            let html = `
                <div style="border-top: 10px solid var(--primary-green); background:var(--card-bg); border-radius:16px; padding:30px; margin-bottom:20px; box-shadow:var(--shadow); border-left:1px solid var(--border-color); border-right:1px solid var(--border-color); border-bottom:1px solid var(--border-color);">
                    <h1 style="font-size:26px; color:var(--text-dark); margin-bottom:5px;">Project Registration Form (${'<?php echo htmlspecialchars($selected_year); ?>'})</h1>
