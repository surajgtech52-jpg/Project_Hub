...
if (isset($_POST['admin_edit_project'])) {
    ...
    $sql = "UPDATE projects SET $leader_sql_update assigned_guide_id=$g_id, department='$dept', topic_1='$t1', topic_2='$t2', topic_3='$t3', extra_data=$extra_json WHERE id = $p_id";
...
if (isset($_POST['upload_file_admin'])) {
    ...
    $stored = store_uploaded_file($file, "admin_req{$req_id}_p{$p_id}");
    if (!$stored['ok']) {
        ...
        $conn->query("INSERT INTO student_uploads (project_id, request_id, file_name, file_path, uploaded_by_name) VALUES ($p_id, $req_id, '$orig', '$path', '$admin_tag')");
...
$all_students_q = $conn->query("SELECT moodle_id, full_name, division, academic_year FROM student WHERE $stu_where ORDER BY academic_year, division, full_name");
$all_members_string = "";
$mem_q = $conn->query("SELECT pm.student_id FROM projects p JOIN project_members pm ON pm.project_id = p.id WHERE $proj_where");
if ($mem_q) {
    while($r = $mem_q->fetch_assoc()) { $all_members_string .= '|' . $r['student_id'] . '|'; }
}

$remaining_students_list = [];
while($st = $all_students_q->fetch_assoc()) {
    // Remaining = not found in any active project's membership table.
    // We avoid moodle_id substring matches; use student id identity by resolving once.
    $sid_q = $conn->query("SELECT id FROM student WHERE moodle_id='".$conn->real_escape_string($st['moodle_id'])."' LIMIT 1");
    $sid = $sid_q && $sid_q->num_rows ? (int)$sid_q->fetch_assoc()['id'] : 0;
    if ($sid === 0 || strpos($all_members_string, '|'.$sid.'|') === false) { $remaining_students_list[] = $st; }
}
...
if ($filter_a_status == 'unassigned') $table_where .= " AND p.assigned_guide_id IS NULL";
if ($filter_a_status == 'assigned') $table_where .= " AND p.assigned_guide_id IS NOT NULL AND p.is_locked = 0";
if ($filter_a_status == 'locked') $table_where .= " AND p.is_locked = 1";
...
$teams_data = [];
$all_projects_query = $conn->query("SELECT p.*, ($md_sql) as member_details, s.full_name as leader_name, s.moodle_id as leader_moodle FROM projects p LEFT JOIN student s ON p.leader_id = s.id");
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <a href="admin_transfer.php" class="nav-link" style="background:transparent;"><i class="fa-solid fa-arrow-right-arrow-left"></i> Year Promotion</a>
    ...
    <div class="main-content">
        ...
        <div id="section-dashboard" class="dashboard-canvas" style="display:none;">
            ...
            <form method="GET" class="search-bar-container">
                ...
                <select name="a_year" class="filter-select" onchange="this.form.submit()">
                    <option value="">All Years</option>
                    <option value="SE" <?php if($filter_a_year=='SE') echo 'selected'; ?>>SE Projects</option>
                    <option value="TE" <?php if($filter_a_year=='TE') echo 'selected'; ?>>TE Projects</option>
                    <option value="BE" <?php if($filter_a_year=='BE') echo 'selected'; ?>>BE Projects</option>
                </select>
        ...
        <div id="section-history" class="dashboard-canvas" style="display:none;">
            <form method="GET" class="search-bar-container">
                ...
                <select name="h_year" class="filter-select" onchange="this.form.submit()">
                    <option value="">All Years</option>
                    <option value="SE" <?php if($filter_h_year=='SE') echo 'selected'; ?>>SE</option>
                    <option value="TE" <?php if($filter_h_year=='TE') echo 'selected'; ?>>TE</option>
                    <option value="BE" <?php if($filter_h_year=='BE') echo 'selected'; ?>>BE</option>
                </select>
            ...
            <iv class="modal-body">
                ...
                <div id="modal-sec-upload" class="tab-content">
                    <div style="background:var(--input-bg); border:1px solid var(--border-color); padding:15px; border-radius:12px; margin-bottom:20px;">
                        ...
                        <form method="POST" style="display:flex; flex-direction:column; gap:10px; margin-top:10px; box-sizing: border-box; width: 100%;">
    ...
    <div id="editFolderModal" class="modal-overlay" style="z-index:2000;">
        <div class="modal-card modal-card-small">
            ...
            <form method="POST" style="display:flex; flex-direction:column; gap:10px; width:100%; box-sizing: border-box;">
                ...
                <input type="text" name="new_folder_name" id="edit_folder_name_input" required style="padding:12px; border:1px solid var(--border-color); border-radius:8px; outline:none; font-family:inherit; background:var(--input-bg); color:var(--text-dark); box-sizing: border-box; width:100%;">
                <label style="font-size:12px; font-weight:600; color:var(--text-light); margin-top:5px;">Note/Instructions</label>
                <textarea name="edit_instructions" id="edit_folder_instructions_input" style="padding:12px; border:1px solid var(--border-color); border-radius:8px; outline:none; font-family:inherit; min-height:80px; resize:vertical; background:var(--input-bg); color:var(--text-dark); box-sizing: border-box; width:100%;"></textarea>
    ...
    <script>
        ...
        function liveActiveProjectSearch(tableId) {
            let studentInput = document.getElementById('searchActiveStudent').value.toLowerCase().trim();
            let guideInput = document.getElementById('searchActiveGuide').value.toLowerCase().trim();
        ...
        function liveHistorySearch(tableId) {
            let studentInput = document.getElementById('searchHistStudent').value.toLowerCase().trim();
            let guideInput = document.getElementById('searchHistGuide').value.toLowerCase().trim();
            let rows = document.querySelectorAll('#' + tableId + ' .searchable-row');
            ...
            rows.forEach(row => {
                ...
                let guideEl = row.querySelector('.guide-name');
                let guideName = guideEl ? guideEl.textContent.toLowerCase() : "";
                let topicEl = row.querySelector('.final-topic');
                let topicName = topicEl ? topicEl.textContent.toLowerCase() : "";

                let studentMatch = studentInput === "" || leaderInfo.includes(studentInput) || memberInfo.includes(studentInput);
                let guideMatch = guideInput === "" || groupName.includes(guideInput) || guideName.includes(guideInput) || topicName.includes(guideInput);
        ...
        function openMasterModal(projectId) {
            ...
            schemaArray.forEach(field => {
                ...
                if (field.type === 'team-members') {
                    ...
                    `;
                    setTimeout(() => {
                        let mc = document.getElementById('admin_membersContainer');
                        mc.innerHTML = '';
                        adminMemberCount = 0;
            ...
            if(data.requests && data.requests.length > 0) {
                data.requests.forEach(req => {
                    ...
                    html += `
                            ...
                            <form method="POST" enctype="multipart/form-data" style="display:flex; flex-direction:column; gap:8px; background:var(--card-bg); padding:15px; border-radius:10px; border:1px dashed var(--border-color); box-sizing:border-box; width:100%;">
        ...
        function addAdminMemberRow(moodle = '', name = '', isLeader = false) {
            const container = document.getElementById('admin_membersContainer');
            const wrapper = document.createElement('div');
            let idx = adminMemberCount++;
        ...
        function togglePassword(inputId, iconId) {
            ...
            if (input.type === "password") {
                input.type = "text";
                icon.classList.remove('fa-eye'); icon.classList.add('fa-eye-slash'); icon.style.color = "var(--primary-green)";
            } else {
                input.type = "password";
                icon.classList.remove('fa-eye-slash'); icon.classList.add('fa-eye'); icon.style.color = "var(--text-light)";
            }
        }
    </script>
</body>
