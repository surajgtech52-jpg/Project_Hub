...
if (isset($_POST['add_student'])) {
    ...
    if (moodle_id_in_use($conn, $m_id)) {
        ...
        $sql = "INSERT INTO student (moodle_id, password, full_name, academic_year, division, phone_number, status) 
                VALUES ('$m_id', '$pass', '$name', '$year', '$div', '$phone', 'Active')";
...
if (isset($_POST['edit_student'])) {
    ...
    if ($current_mid !== null && $m_id !== $current_mid) {
        ...
        $sql = "UPDATE student SET full_name='$name', academic_year='$year', division='$div', phone_number='$phone', status='$status' WHERE id=$id_int";
...
if ($filter_year) {
    $where .= " AND s.academic_year = '$filter_year'";
}
...
<body>
            ...
            <form method="GET" style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 20px; flex-wrap:wrap; gap:15px;">
                <div style="display:flex; gap:10px; flex:1; min-width:300px;">
                    <select name="year" class="filter-select" onchange="this.form.submit()">
                        ...
                        <option value="SE" <?php if($filter_year=='SE') echo 'selected'; ?>>SE Students</option>
                        <option value="TE" <?php if($filter_year=='TE') echo 'selected'; ?>>TE Students</option>
                        <option value="BE" <?php if($filter_year=='BE') echo 'selected'; ?>>BE Students</option>
                    </select>
                ...
                <table id="studentsTable">
                    ...
                    <tbody style="padding:15px;">
                        ...
                        <tr class="student-row">
                            ...
                            <td style="padding:15px;">
                                <span style="font-size:14px; font-weight: 600; color: var(--text-dark);"><?php echo htmlspecialchars($row['academic_year']); ?> - Div <?php echo htmlspecialchars($row['division']); ?></span><br>
                            ...
                            <td style="text-align:center; padding:15px; vertical-align:middle;">
                                <button class="btn-edit" onclick="openEditModal(<?php echo $row['id']; ?>, '<?php echo htmlspecialchars($row['moodle_id']); ?>', '<?php echo htmlspecialchars($row['full_name']); ?>', '<?php echo htmlspecialchars($row['academic_year']); ?>', '<?php echo htmlspecialchars($row['division']); ?>', '<?php echo htmlspecialchars($row['phone_number']); ?>', '<?php echo htmlspecialchars($row['status']); ?>')">
        ...
        <div class="modal-card modal-card-small">
            ...
            <form method="POST">
                ...
                <div style="display:flex; gap:10px;">
                    <div class="form-group" style="flex:1;">
                        <label>Year</label>
                        <select name="academic_year" id="edit_year" required>
                            <option value="SE">SE</option><option value="TE">TE</option><option value="BE">BE</option>
                        </select>
                    </div>
                    <div class="form-group" style="flex:1;">
    ...
    <script>
        ...
        function openEditModal(id, moodle, name, year, div, phone, status) {
